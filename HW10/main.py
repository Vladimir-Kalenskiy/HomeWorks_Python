...
def start(update, context):                 # Начало работы калькулятора.
    context.bot.send_message(update.effective_chat.id, 'Вас приветствует калькулятор!')

    get_id_user(update.effective_chat.id)
    return start_calc

def receiving_data(update, context):         # Логика бота
...
def cancel(update, context):                 # Закрытие бота
    context.bot.send_message(update.effective_chat.id, 'Good bye!')
